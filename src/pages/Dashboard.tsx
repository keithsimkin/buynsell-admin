...
function TileLink({
  ...
  return (
    <Link
      to={to}
      className="group flex min-h-[118px] flex-col justify-between rounded-2xl border border-bn-border/80 bg-white p-4 shadow-[var(--shadow-bn-sm)] transition hover:-translate-y-0.5 hover:border-bn-yellow hover:shadow-[var(--shadow-bn-md)]"
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-[12px] font-semibold uppercase tracking-wide text-bn-muted">{label}</p>
        <ChevronRight className="h-4 w-4 text-neutral-300 transition group-hover:translate-x-0.5 group-hover:text-bn-ink" />
      </div>
      <div>
        <p className="text-[28px] font-bold tracking-tight text-bn-ink">{value}</p>
        <p className={`mt-1 text-[11px] ${pulse ? 'font-semibold text-bn-orange' : 'text-bn-muted'}`}>
          {hint}
        </p>
      </div>
    </Link>
...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Workboard header */}
      <section className="relative overflow-hidden rounded-3xl bg-bn-ink px-5 py-6 text-white sm:px-7">
        <div
          className="pointer-events-none absolute -right-10 -top-16 h-56 w-56 rounded-full opacity-50"
          style={{ background: 'radial-gradient(circle, #ffde34 0%, transparent 70%)' }}
        />
        <div
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              to="/listings?status=inreview"
              className="inline-flex items-center gap-1.5 rounded-full bg-bn-yellow px-4 py-2 text-[12px] font-semibold text-bn-ink transition hover:bg-bn-yellow-dark"
            >
      ...
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        ...
        <Card className="bg-gradient-to-br from-gray-50 to-white p-5 lg:col-span-2">
          ...
          <div className="mt-4 grid grid-cols-1 gap-2">
            {(
              ...
              <Link
                key={to}
                to={to}
                className="flex items-center gap-3 rounded-2xl border border-bn-border/80 bg-white px-3 py-2.5 text-[13px] font-medium transition hover:border-bn-yellow hover:bg-bn-yellow/20"
              >
          ...
          <div className="mt-4 rounded-2xl border border-bn-border/70 bg-white/80 px-3 py-2.5 text-[11px] text-gray-500">
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-50 last:border-0 ${
                      href ? 'cursor-pointer hover:bg-bn-yellow/25' : ''
                    }`}
                    onClick={() => {
